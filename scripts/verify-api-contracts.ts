...
process.env.HUBSPOT_DEMO_MODE = 'true';
...
async function verifyChatCompletionsMissingEnv() {
  const { createChatCompletionsHandler } =
    await import('../lib/sales/chat-handler');
...
async function verifyCrmSyncLeadRoute() {
  const { POST: syncLeadPost } = await import('../app/api/crm/sync-lead/route');

  // 1. Validation failure on missing sessionId
  ...
  const validReq = new NextRequest('http://localhost:3000/api/crm/sync-lead', {
    ...
    body: JSON.stringify({
      ...
      customerData: { name: 'Contract Test User', email: 'test@example.com' },
    }),
  });
  const validRes = await syncLeadPost(validReq);
  assert(validRes.status === 200, 'POST /api/crm/sync-lead should return 200 for valid session');
  const validBody = await getJson(validRes);
  assert(validBody.success === true, 'POST /api/crm/sync-lead should return success: true');
}
