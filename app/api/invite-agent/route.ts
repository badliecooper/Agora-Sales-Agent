...
import { buildSalesAgentPrompt } from '@/lib/knowledge/agent-context';

// System prompt that defines the agent's personality and behavior.
// Swap this out to change what the agent talks about.
const _ADA_PROMPT = `You are **Ada**, an agentic developer advocate from **Agora**. You help developers understand and build with Agora's Conversational AI platform.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { requester_id, channel_name, companyId = 'default-company' } = body;
    ...
    // --- 2. Build and start the agent with knowledge base context ---
    const instructions = buildSalesAgentPrompt({ companyId, sessionId: channel_name });
    console.log(`[invite-agent] Injected compiled knowledge base for tenant: ${companyId}, session: ${channel_name} (${instructions.length} chars)`);
    ...
    const agent = new Agent({
      ...
      turnDetection: {
        config: {
          ...
          end_of_speech: {
            mode: 'vad',
            vad_config: {
              silence_duration_ms: 800, // 800ms natural conversational pause prevents premature turn-ending and duplicate prompts
            },
          },
        },
      },
      ...
      parameters: {
        // web client → ultra-low-latency chorus profile with direct RTC datastream transcripts
      ...
      .withLlm(
        process.env.NEXT_LLM_URL && process.env.NEXT_LLM_API_KEY
          ? new OpenAI({
    ...
    // Store active session on globalThis so SalesState updates can sync directly with the live agent
    const globalSessions = globalThis as unknown as {
      activeAgentSessions?: Map<string, typeof session>;
    };
    if (!globalSessions.activeAgentSessions) {
      globalSessions.activeAgentSessions = new Map();
    }
    globalSessions.activeAgentSessions.set(channel_name, session);
    console.log(`[invite-agent] Registered active AgentSession for channel: ${channel_name}`);
