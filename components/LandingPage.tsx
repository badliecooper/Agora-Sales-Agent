...
export default function LandingPage() {
      ...
      const [agentData, rtm] = await Promise.all([
        ...
        (async (): Promise<RTMClient | null> => {
          ...
          const rtmSetup = async (): Promise<RTMClient | null> => {
            ...
            rtm.addEventListener('status', (statusEvent) => {
              console.log('[RTM Status]', statusEvent);
            });
            console.log('[RTM] Logging in...');
            try {
              const rtmToken = (responseData as { rtmToken?: string }).rtmToken || responseData.token;
          ...
          try {
            const timeout = new Promise<null>((resolve) =>
              setTimeout(() => {
                console.warn(
                  `[RTM] Setup timed out after ${RTM_TIMEOUT_MS / 1000}s. Gracefully continuing in RTC voice mode.`,
                );
                resolve(null);
              }, RTM_TIMEOUT_MS),
            );
  ...
  const handleEndConversation = async (
    ...
    const channel = agoraData?.channel;
    // Strictly idempotent post-call HubSpot CRM sync (max once per channel session)
    if (channel && !syncedChannelsRef.current.has(channel)) {
      syncedChannelsRef.current.add(channel);
      console.log(
        '[LandingPage] Triggering CRM sync with transcript turns:',
        transcript?.length || 0,
      );
      fetch('/api/crm/sync-lead', {
        ...
        console.warn(
          '[CRM] Background post-call lead sync error (non-fatal):',
          err,
        );
      });
    }
